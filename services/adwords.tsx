...
export function useFetchKeywordIdeas(router: NextRouter, _adwordsConnected = false) {
   ...
   return useQuery({
      queryKey: ['keywordIdeas', domainSlug ?? ''],
      queryFn: () => {
         if (!domainSlug) { throw new Error('Missing domain slug'); }
         return fetchAdwordsKeywordIdeas(router, domainSlug);
      },
      enabled,
      retry: false,
   });
...
export function useMutateKeywordIdeas(router:NextRouter, onSuccess?: Function) {
   ...
   return useMutation({
      ...
      onSuccess: async (data) => {
         ...
         queryClient.invalidateQueries({ queryKey: ['keywordIdeas', domainSlug ?? ''] });
      },
      onError: (error, _variables, _context) => {
         console.log('Error Loading Keyword Ideas!!!', error);
