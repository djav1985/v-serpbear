...
export const extractLocalResultsFromPayload = (payload: unknown, debug = false): LocalResultEntry[] => {
   ...
   register((container.local_results as { results?: unknown[] })?.results);
   register((container.local_results as { local_results?: unknown[] })?.local_results);
   register((container.local_results as { places?: unknown[] })?.places);
   register((container.local_pack as { results?: unknown[] })?.results);
   ...
   register((container.results as { local_results?: unknown[] })?.local_results);
   // Mobile-specific variations
   register(container.mobile_local_results);
   register(container.local_map);
   register((container.local_map as { places?: unknown[] })?.places);
